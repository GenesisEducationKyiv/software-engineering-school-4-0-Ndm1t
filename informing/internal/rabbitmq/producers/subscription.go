...
func NewEmailProducer(conn *amqp.Connection, topic string) (*Producer, error) {
	ch, err := conn.Channel()
	if err != nil {
...
func (p *Producer) Publish(eventType string, email models.Subscription, ctx context.Context) error {
	emailMessage := rabbitmq.NewSubscriptionMessage(eventType, email)
