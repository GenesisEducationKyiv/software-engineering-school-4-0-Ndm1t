...
import (
	...
	"rate-service/internal/rabbitmq"
	"time"
)
...
func NewRateProducer(conn *amqp.Connection, topic string) (*Producer, error) {
	ch, err := conn.Channel()
	if err != nil {
...
func (p *Producer) Publish(eventType string, rate models.Rate, ctx context.Context) error {
	emailMessage := rabbitmq.NewRateMessage(eventType, rate)
